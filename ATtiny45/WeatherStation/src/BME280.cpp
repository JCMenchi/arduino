...
#define PRES_SAMPLING 3 // use 4x sampling
...
/****************************************************************/
void BME280::WriteSettings(bool filter_off) {
  ...
  config = (BME280_STANDBY_TIME << 5);

  if (filter_off) {
    config |= (BME280_FILTER_OFF << 2);
  } else {
    config |= (BME280_FILTER << 2);
  }
...
bool BME280::ReadData(int32_t data[SENSOR_DATA_LENGTH]) {
  ...
  // Registers are in order. So we can start at the pressure register and read 8
  // bytes.
...
/****************************************************************/
int32_t BME280::CalculateTemperature(int32_t raw, int32_t &t_fine) {
...
/****************************************************************/
uint16_t BME280::CalculateHumidity(uint32_t raw, int32_t t_fine) {
  // Code based on calibration algorthim provided by Bosch.
  int32_t var1;
...
/****************************************************************/
int32_t BME280::CalculatePressure(int32_t raw, int32_t t_fine) {
  // Code based on calibration algorthim provided by Bosch.
...
/****************************************************************/
int32_t BME280::pres(int32_t &temp) {
  int32_t data[8];
  int32_t t_fine;
  if (!ReadData(data)) {
    return -1;
...
/****************************************************************/
uint16_t BME280::hum(int32_t &temp) {
  int32_t data[8];
  int32_t t_fine;
  if (!ReadData(data)) {
    return -1;
...
/****************************************************************/
void BME280::read(int32_t &pressure, int32_t &temp, uint16_t &humidity) {
  int32_t data[8];
  int32_t t_fine;
  if (!ReadData(data)) {
    return;
