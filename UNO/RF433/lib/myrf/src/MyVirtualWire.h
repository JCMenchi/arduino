...
// DIRECTLY: USE THE LISTS Copyright (C) 2008 Mike McCauley $Id:
// VirtualWire.h,v 1.14 2014/03/26 01:09:36 mikem Exp mikem $
...
/// VirtualWire, and much more besides, including Reliable Datagrams,
/// Addressing, Routing and Meshes. All the platforms that VirtualWire supported
/// are also supported by RadioHead.
///
/// This library will no longer be maintained or updated, but we will continue
/// to publish it for the benefit of the the community. Nevertheless we
/// recommend upgrading to RadioHead where possible.
///
/// VirtualWire is a library for Arduino, Maple and others that provides
/// features to send short messages, without addressing, retransmit or
/// acknowledgment, a bit like UDP over wireless, using ASK (amplitude shift
/// keying). Supports a number of inexpensive radio transmitters and receivers.
/// All that is required is transmit data, receive data and (for transmitters,
/// optionally) a PTT transmitter enable. Can also be used over various analog
/// connections (not just a data radio), such as the audio channel of an A/V
/// sender
///
...
/// downloaded from
/// http://www.airspayce.com/mikem/arduino/VirtualWire/VirtualWire-1.27.zip You
...
/// licensing. Before asking a question or reporting a bug, please read
...
/// with MSP430G2553 / G2452 and Arduino with ATMega328 (courtesy Yannick DEVOS
/// - XV4Y), but untested by us. It also runs on Teensy 3.0 and 3.1 (courtesy of
/// Paul Stoffregen) using the Arduino IDE 1.0.5 and the Teensyduino addon 1.18.
...
/// connecting pin 12 of one to 11 of the other and vice versa, like this for a
/// duplex connection:
///
/// <code>
/// <pre>
...
/// transmitter/receiver, such as the audio channel of an A/V
/// transmitter/receiver. You may need buffers at each end of the connection to
/// convert the 0-5V digital output to a suitable analog voltage.
///
...
/// value="VirtualWire" /> <input type="hidden" name="currency_code" value="USD"
...
/// \par Trademarks
///
/// VirtualWire is a trademark of AirSpayce Pty Ltd. The VirtualWire mark was
/// first used on April 20 2008 for international trade, and is used only in
/// relation to data communications hardware and software and related services.
/// It is not to be confused with any other similar marks covering other goods
/// and services.
///
/// \par Copyright
///
/// This software is Copyright (C) 2011-2014 Mike McCauley. Use is subject to
/// license conditions. The main licensing options available are GPL V2 or
/// Commercial:
///
...
/// \version 1.11 Converted documentation to Doxygen. Moved CHANGES log to this
/// version history.
///     Ensure vw_rx_pin is not accessed unless receiver is enabled
/// \version 1.12 Compiles and runs on on Energia with MSP430G2553 / G2452 and
/// Arduino with ATMega328.
///     Patches contributed by Yannick DEVOS - XV4Y
/// \version 1.13 util/crc16.h needed for compiling on  Energia with MSP430G2553
/// / G2452 was accidentally
///     left out of the distribution
/// \version 1.14 Added support ATtiny85 on Arduino, patch provided by r4z0r7o3.
/// \version 1.15 Updated author and distribution location details to
/// airspayce.com \version 1.16 Added support for Teensy 3.0, contributed by
/// Paul Stoffregen. \version 1.17 Increase default MAX_MESSAGE_LENGTH to 80.
/// Added vw_get_rx_good() and vw_get_rx_bad()
///               functions.
/// \version 1.18 Added support for Maple, Flymaple etc with STM32F103RE
/// processor using timer 1.
///               Tested with Flymaple 1.1 and maple-ide-v0.0.12
/// \version 1.19 Added new function vw_rx_inverted(), to allow the incoming RX
/// to be inverted (normally high).
///               Minor improvements to timer setup for Maple. Name
///               vw_tx_active() changed from incorrect vx_tx_active()
/// \version 1.20 Added support for ATtiny84, patched by Chuck Benedict.
/// \version 1.21 Added support for most AVR8 platforms with proper
/// configuration, without depending
///               on Arduino environment, such as Atmega32u2, Atmega32U4,
///               At90USB162 etc, contributed by Alexandru Daniel Mircescu.
/// \version 1.22 Alexandru Daniel Mircescu fixed some problems with the
/// recently added AtMega32U2 support. \version 1.23 Fixed some errors and
/// inaccuracies in documentation, with the kind assistance of Alexandru
///               Mircescu.
/// \version 1.24 Minor performance improvement to vw_symbol_6to4() courtesy
/// Ralph Doncaster. \version 1.25 Removed all use of floating point code, with
/// assistance of a patch from Jim Remington.
///               Result is smaller code size.
/// \version 1.26 Removed util/crc16.h from distribution, since it is now
/// included in arduino IDE
///               since version 1.0. Support for arduino IDE prior to 1.0 is now
///               abandoned.
/// \version 1.27 Reinstated VWutil/crc16.h for the benefit of other platforms
/// such as Teensy.
///               Testing on Teensy 3.1. Added End Of Life notice. This library
///               will no longer be maintained and updated: use RadioHead
///               instead.
///
...
#ifndef MyVirtualWire_h
#define MyVirtualWire_h

#include "MyVirtualWire_Config.h"
#include <stdint.h>
...
extern void myvw_setup(uint16_t speed);
...
extern void myvw_rx_stop();

/// Block until a message is available
/// then returns
extern void myvw_wait_rx();
...
extern uint8_t myvw_wait_rx_max(unsigned long milliseconds);

/// Returns true if an unread message is available
/// \return true if a message is available to read
extern uint8_t myvw_have_message();

/// If a message is available (good checksum or not), copies
/// up to *len octets to buf.
/// \param[in] buf Pointer to location to save the read data (must be at least
/// *len bytes. \param[in,out] len Available space in buf. Will be set to the
/// actual number of octets read \return true if there was a message and the
/// checksum was good
extern uint8_t myvw_get_message(uint8_t *buf, uint8_t *len);
...
extern uint8_t myvw_get_rx_good();
...
/// \return true if the message was accepted for transmission, false if the
/// message is too long (>VW_MAX_MESSAGE_LEN - 3)
extern uint8_t myvw_send(uint8_t *buf, uint8_t len);
