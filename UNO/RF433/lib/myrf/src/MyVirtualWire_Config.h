...
//////////////////////////////////////////////////////////////////////////
//	The following configurations are intented only if VW_PLATFORM ==
...
//	The default timer configuration will use TIMSKn as interrupt mask
//register,
...
/*
        ...
        //	My radio doesn't have PTT feature => VW_PTT_PORT/DDR/PIN will be
   left undefined
        //#define VW_PTT_PORT
        //#define VW_PTT_DDR
        //#define VW_PTT_PIN
...
//	Instead of defining VW_PTT_PORT/DDR/PIN, VW_TX_PORT/DDR/PIN or
//VW_RX_PORT/DDR/PIN 	   the user can use the following preprocessor directives to
//control the way 	   VirtualWire library reads the RX and writes to the TX or PTT
//
...
//
//	EXAMPLE: Advanced pin configuration that lights up a LED when PTT is
//high
...
#define vw_digitalRead_rx() (PORTD & (1 << PORTD7) ? 1 : 0)
#define vw_digitalWrite_tx(value)                                              \
...
//	If the default timer setup doesn't suite the platform you are using, you
//can 	   rewrite the timer setup routine and define the timer vector used by the
//ISR 	   implemented within the library
//
...
static inline void my_vw_timerSetup(uint16_t speed)
    __attribute__((always_inline));

//	VirtualWire has a special routine for detecting prescaler and the number
// of ticks automatically, but needs to be declared first in order to be used
extern uint8_t myvw_timer_calc(uint16_t speed, uint16_t max_ticks, uint16_t *nticks);

//	Instruct VirtualWire to use my timer setup routine and my interrupt
//vector
...
static inline void my_vw_timerSetup(uint16_t speed) {
  ...
  uint16_t nticks;
  uint8_t prescaler = myvw_timer_calc(speed, (uint16_t)-1, &nticks);
  if (!prescaler) {
    return; // fault
  }
