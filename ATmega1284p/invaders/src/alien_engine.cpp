...
void alien_hit_something(uint8_t x, uint8_t ymin, uint8_t ymax, CH1115Display *display) {
    ...
    else if (ymax >= 56 && x < x_spaceship_position + SPRITE_WIDTH && x > x_spaceship_position) {
#ifdef HAS_SERIAL
...
void do_update_alien(CH1115Display *display) {
    ...
    if (alien_missile_state != -1) {
        ...
        if (prev != 0) {
            ...
            USART_WriteString("\n");
#endif
            alien_hit_something(alien_x_missile, alien_y_missile, alien_y_missile + 4, display);
        }
...
#ifdef HAS_SERIAL
            USART_WriteString("Clear alien missile hit at: ");
            USART_WriteInt(alien_x_missile);
            USART_WriteString(", ");
            USART_WriteInt(alien_y_missile);
            USART_WriteString("\n");
