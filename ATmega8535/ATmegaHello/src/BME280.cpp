...
static uint32_t CalculatePressure(int32_t raw, int32_t t_fine) {
  // Code based on 8bit calibration algorithm provided by Bosch.
  int32_t var1, var2;
  uint32_t p;
...
uint32_t BME280_pres(int32_t &temp) {
...
/****************************************************************/
void BME280_read(uint32_t &pressure, int32_t &temp, uint16_t &humidity) {
  uint8_t data[SENSOR_DATA_LENGTH];
  ReadRegister(PRESS_ADDR, data, SENSOR_DATA_LENGTH);
